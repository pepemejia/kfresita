...
async function loadOrders() {
  if (adminState.mode === 'supabase' && supabase) {
...
async function loadMenu() {
  ...
  adminState.activeMenu = safeArray(overrideMenu).length ? overrideMenu.map(normalizeProduct) : [...adminState.baseMenu];
}
...
function filteredOrders() {
  return adminState.orders.filter((order) => adminState.orderFilter === 'all' ? true : order.status === adminState.orderFilter);
}
async function updateOrderStatus(orderId, status) {
  ...
  adminState.orders = adminState.orders.map((order) => order.id === orderId ? { ...order, status } : order);
...
function renderOrders() {
  ...
  orders.forEach((order) => {
    ...
    const itemsHtml = safeArray(order.items).map((item) => `<li>${item.title} × ${item.quantity} — ${money(item.price * item.quantity)}</li>`).join('');
    card.innerHTML = `
...
function renderProducts() {
  ...
  list.querySelectorAll('.toggle-product-btn').forEach((btn) => {
    btn.addEventListener('click', async () => {
      adminState.activeMenu = adminState.activeMenu.map((item) => item.id === btn.dataset.id ? { ...item, active: item.active === false } : item);
      await persistMenu();
    });
  });
...
async function setupAuth() {
  ...
  $('logoutAdmin')?.addEventListener('click', async () => {
    if (supabase && adminState.mode === 'supabase') await supabase.auth.signOut();
...
function setupProducts() {
  $('productForm')?.addEventListener('submit', async (event) => {
    ...
    adminState.activeMenu.push(normalizeProduct({ id: crypto.randomUUID(), title, description, price, image, category, active: true }));
    await persistMenu();
    $('productForm').reset();
  });
